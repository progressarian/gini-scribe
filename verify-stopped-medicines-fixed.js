...
async function verify() {
  try {
    ...
    inactiveMeds.forEach((m, i) => {
      ...
      console.log(
        `     Stopped: ${m.stopped_date ? m.stopped_date.toISOString().split("T")[0] : "N/A"}`,
      );
    ...
    const nmz10 = inactiveMeds.find(
      (m) => m.name.toUpperCase().includes("NMZ") && m.dose === "10mg",
    );
    ...
    if (nmz10) {
      ...
      const nmz10_alt = inactiveMeds.find(
        (m) => m.name.toUpperCase().includes("NMZ") && (m.dose === "10" || m.dose === "10 mg"),
      );
      if (nmz10_alt) {
