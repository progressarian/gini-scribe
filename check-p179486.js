...
async function check() {
  try {
    ...
    apptRes.rows.forEach((appt, i) => {
      ...
      const hasNotes = appt.healthray_clinical_notes
        ? `${appt.healthray_clinical_notes.length} chars`
        : "NULL";
      console.log(
        `${i + 1}. [${appt.appointment_date}] HealthRay ID: ${appt.healthray_id || "NULL"}`,
      );
    ...
    const dxRes = await pool.query(`SELECT * FROM diagnoses WHERE patient_id = $1`, [patient.id]);
