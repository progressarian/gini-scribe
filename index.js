...
app.get("/api/health", async (req, res) => {
  try {
    ...
    res.json({
      status: "ok",
      ...result.rows[0],
      db_url_prefix: (process.env.DATABASE_URL || "").slice(0, 50) + "...",
    });
  } catch (e) {
...
app.get("/api/patients", async (req, res) => {
  try {
    ...
    if (doctor) {
      conditions.push(
        `EXISTS (SELECT 1 FROM consultations c WHERE c.patient_id=p.id AND c.con_name ILIKE $${idx})`,
      );
      params.push(`%${doctor}%`);
      idx++;
    }
...
app.get("/api/patients/:id", async (req, res) => {
  try {
    ...
    const [consultations, vitals, meds, labs, diagnoses, docs, goals] = await Promise.all([
      pool.query(
        "SELECT id, visit_date, visit_type, mo_name, con_name, status, created_at FROM consultations WHERE patient_id=$1 ORDER BY visit_date DESC, created_at DESC",
        [id],
      ),
    ...
    console.error("Patient detail error:", e.message);
    res.status(500).json({ error: e.message });
  }
});
...
app.get("/api/patients/:id/outcomes", async (req, res) => {
  try {
    ...
    console.error("Outcomes error:", e.message);
    res.status(500).json({ error: e.message });
  }
});
