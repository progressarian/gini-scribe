...
- [x] **P4-26 · Billing routes** — `Done`
  ...
  - **Not yet run:** the 13 route tests need the test API on 3101 restarted to serve the new routes. They were instead proved through a throwaway harness that mounts the real router on an ephemeral port (~45 checks green twice, both PDFs rendering real `%PDF` bytes, and the full 403 sweep: all 24 desk endpoints refused for a coordinator, all 5 master endpoints refused for reception). Run after restarting the API: `DATABASE_URL=postgres://user:pass@localhost:5435/gini_scribe_test npx playwright test --config <pure config> e2e/billing/phase4/P4-26-billing-routes.spec.js`.
