...
- [x] **P2b-12 · Daily sweep of expired imports** — `Done` (follow-up to P2b-07)
  - **Where:** `server/services/cron/importSessionSweep.js`, registered in `startCronJobs()` (`server/services/cron/index.js`) with lock key `CRON_LOCK_KEYS.IMPORT_SESSION_SWEEP` in `lowPriority.js`.
  - **Why:** `purgeStaleSessions` ran only on the next upload, so an expired or abandoned session — its stored file (up to 5 MB) and its rows — stayed until someone uploaded again.
  - **E2E test:** `e2e/billing/phase2b/P2b-12-daily-sweep.spec.js`.
  - **Result:** Done 2026-09-25 (built by a sub-agent in an isolated worktree, merged and verified). Runs 25 minutes after the worker starts, then every 24 hours, under its own advisory lock, the same idiom as the auth-token cleanup; logs only when it removes something; a failure is caught and never crashes the worker. 3 tests (expired and abandoned removed while open and committed are kept; a second run removes nothing and logs nothing; a held lock skips the run). **The worker must be restarted for it to start running.** Same change: **P1-24** now removes everything it creates (it had no teardown — the source of the leftover Follow Up consultation item that failed its own next run), sweeps earlier crashed runs, and restores the VPT item it changes; **P1-25** always removes its `RCPT / 2051-52` series row. P2b-12 + P1-24 + P1-25: 19/19, twice, with nothing left behind.
...
- [x] **P4-31 · Discount code box** — `Done`
  ...
  - **Review:** (2026-09-24), run against the test servers. No code changes. The accept path replaces the whole working copy from the server's response, and a refusal is the server's own sentence — unknown code, nothing taken off, past its daily limit — never a generic one. Applied codes are removable chips, automatic discounts are named and have no remove control, and the test that asserts the _absence_ of any manual discount field still holds. 5 tests, green twice.
...
- [ ] **P4-38 · Floor trial** — `Pending`
  ...
  - **Rehearsal:** (2026-09-25, by a sub-agent) on the test system, the whole of `52-BILLING-FLOOR-TRIAL.md` driven through the real screens — reception at the counter, reception_admin on Desk requests, six patients checked in on Arrivals, every row of the checklist table. Encoded as `P4-38-floor-trial-rehearsal.spec.js` (seeded by `p438-floor-day.mjs`) to re-run before the real day. **Two problems would each have stopped the real trial on the first patient:** (1) a check-in on the **Arrivals** screen, and a HealthRay-synced patient, got no draft or consultation — P4-07 had hooked only two of the check-in paths; `journey.js` now drafts at check-in and a new `consultationForDesk` adds the doctor's consultation when the counter opens a visit (once per visit; not again after the desk removes it; again after a cancel) — `P4-38b`, 5 tests, server-side, **needs an API restart**; (2) a **General (no-category) patient could never be finalised** — the counter demanded a category and offered no General answer, hidden because every earlier spec set one. Seven more fixed on the counter, each with a failing test first (`P4-38c`, 10 tests): a test ordered while the bill was open never appeared; no way to reach a second bill's draft; earlier bills could not be reopened to cancel or reprint; no way forward after a cancel; an accepted code showed only its letters; an empty draft said no payment was needed; stale item-search results could add the wrong item. The checklist was corrected (General is not a category; what deploying turns on; setting labels; the storage and restart preconditions; the supervised realtime check) and a Rehearsal section added. The rehearsal passed twice with the API restart simulated. **Verdict: ready for a supervised trial** once the build is deployed with an API restart, a referral scan upload is confirmed on production, and **Settings → Services → Not priced** is empty for the day's tests. **Open:** an unpriced test on the same order as a priced one is released and recorded paid (`settleOrder`) — P4C-04; referral scans need Supabase storage, and the upload also runs extraction and pushes to the patient app — a decision; `openDraft` opens an empty draft for a visit whose bills are all final; shared-DB interference between billing specs (`billing_settings`, the reception user's shifts, the fixture's bill series). **The real floor trial is still to run.**
...
AND is_live` inside the transaction `clearPayment` already holds.
  ...
  - **Follow-up (closed by P4-41, 2026-09-24) — the hospital can collect twice:** on the claim route the patient pays the full ₹250 on the bill while the order's own ₹150 insurance claim still stands, so the hospital is paid ₹400 for a ₹250 test (the patient is not overcharged; the payer is). Not fixed here; needs its own task — the bill should either honour the order's standing claim or the claim should be withdrawn when the bill takes the test.
...
- [x] **P4-42 · Reception money on the bill** — `Done` (found by P4-41)
  - **Where:** `server/services/billing/payments.js` (`refuseReceptionMoney`, `orderStatesOn`), `bills.js` (`shapeLine`, `shownLines`), `shared/billingVocab.js` (`ORDER_STATE`), `src/components/billing/counter/BillLinesTable.jsx`, `lineText.js`.
  - **Why:** P4-41 closed the insurer-claim route but not the cash one — a cancelled bill, reception collecting ₹250 cash on the order, and the counter re-adding that test by its order let the bill take ₹250 again: ₹500 for a ₹250 test. And the counter only learnt a test belonged to reception when payment or finalise was refused.
  - **What:** the bill refuses to take money for, or finalise with, a live test line whose order carries money this bill did not write — a standing claim (as P4-41) or cash paid at reception — unless the order's money is still exactly what this bill's own settle left. Each line carries `order_state` (`claim_at_reception` / `paid_at_reception` / null) from the same rows as the guard, and the counter shows it up front.
  - **E2E test:** `e2e/billing/phase4/P4-42-reception-money-on-the-bill.spec.js`.
  - **Result:** Done 2026-09-25 (built by a sub-agent in an isolated worktree, merged and verified on the main checkout). `refuseStandingClaims` became `refuseReceptionMoney`: one query over the bill's live test lines and their orders, flagging `claim_at_reception` when a submitted or approved claim stands, else `paid_at_reception` when `amount_paid > 0`, unless the order's money equals this bill's own last settle. It runs in `takePayments` before any money is taken and in `finaliseBill` before a number is taken. The claim refusal is P4-41's word for word (`code: "order_claim"`); the cash refusal is a 409 with `code: "order_paid"` — "HbA1c was already paid ₹250.00 at reception, so it can't also be paid on this visit's draft bill — remove it from this bill" (adding "and collect the rest at reception" when part paid; on a final bill "cancel this bill and bill it again without this test"). Every bill-returning call gives each line `order_state` from the same rows, so the flag and the guard cannot disagree; the counter shows it as an amber badge with a one-line note. P4-40 test 2, which asserted the old ₹500, now asserts the refusal and ₹250. No schema, pricing, `clearPayment` or `labPayment.js` change — P4-18's pins hold. **First run on the main checkout:** 66 of 69 passed across P4-13/14/15/16/17/18/24/40/41/42; the one failure (test 5, ₹500 expected ₹250) was the spec's own `hospitalHolds` helper counting the bill's payment twice — once on the bill and again as the `amount_paid` the bill's own settle mirrors onto the order to open the lab gate. The helper now reads the settle event's `before`/`after` and counts only money reception really holds. Then P4-42 + P4-40 + P4-41: 18/18, twice. Two deliberate breaks (dropping the `paid_at_reception` branch; dropping the guard from `takePayments`) each failed a test and were restored by md5. 7 tests. The API needs a restart for the counter to show the flag over HTTP.

---

## Phase 4c — Billing Counter go-live

Goal: everything the floor trial (P4-38) would otherwise trip over — how the
bill looks on paper, GST on bills printed later, realtime in a real browser,
and a full rehearsal of the trial day on the test system.

- [x] **P4C-01 · Printed bill check** — `Done`
  - **Where:** `server/services/billing/billPdf.js`; `e2e/billing/phase4/P4-23-print-layout.spec.js`.
  - **Result:** Done 2026-09-25 (built by a sub-agent). The first time anyone _looked_ at a printed bill: real PDFs of 11 bill and receipt kinds, lakh-amount and unbroken-word variants, and a 94-case sweep of bill lengths 1–48 with GST on and off, every page converted to PNG and inspected. Six problems fixed in the page's own CSS: **on a GST bill with large amounts the "Patient pays" column was cut off at the page edge**, and the item column crushed to a word per line — a compact padding rule for the GST table never applied (equal specificity), now `table.bp-lines.bp-gst`; a long unbroken word stretched the table and clipped five columns (`overflow-wrap`, with SAC/HSN kept whole); a row split across a page break (`tr{break-inside:avoid}`); and the totals and footer split across pages or stranded alone — a CSS keep-together does not work after a table in Chrome, so the totals and footer are now the items table's last row, which keeps them with the last line and repeats the header above them (the look unchanged). The sweep went from 14/80 bad to 0/94. 3 tests. Before/after images in the scratchpad `print-check/look/`.
  - **Review:** (2026-09-25) Every page of every bill kind looked at again. One real problem remained: `overflow-wrap:anywhere` let the GST table squeeze the Item column to 51 px and **split ordinary words** ("Continuou|s", "Consultati|on") on a document the patient takes home — without splitting words the table is ~6 px too wide for the page, so the builder's version only fit because it split them. The item cell now has `min-width:8em` (`anywhere` kept, so an unbroken string still can't cut off the table) and the GST table's side padding went from 4 px to 3 px: words wrap whole, only a genuinely unbroken string breaks, and it fits up to ₹9,99,999.99 a line, also in fallback fonts. The builder's untested break was real — removing the closing row's `break-before:avoid` strands the totals and footer on their own page at 27 of 94 lengths while the single 46-line bill hid it; a new test prints every length from 1 to 46. Test 1 now fails on any split word of 16 characters or fewer. **For a decision:** bills load Google Fonts at print time and the renderer waits up to 30 s for the network to go quiet — a slow Google Fonts printed one bill in fallback fonts and timed out one sweep; bundling the fonts locally would remove this (it affects prescriptions too).
- [x] **P4C-02 · GST as issued** — `Done (test database; not yet on production)`
  - **Where:** `server/migrations/2026-10-23_billing_bill_gst_snapshot.sql`; `bills.js` (`finaliseBill`), `creditNotes.js`, `billPdf.js`; `e2e/billing/phase4/P4-23-gst-as-issued.spec.js`.
  - **Result:** Done 2026-09-25 (built by a sub-agent). A bill now prints as it was issued: `bills.issued_gst`, `issued_gstin` and `issued_legal_name` are written by `finaliseBill` from the settings it already reads, and credit notes copy their bill's snapshot — so a bill finalised before GST registration no longer grows empty tax columns and a GSTIN when reprinted after, and a later GSTIN or legal-name change never reaches an old bill. Checks: a draft carries no snapshot; a GSTIN only with GST on. Existing final bills are backfilled from the settings as they stood at their `finalised_at` (the append-only settings audit history), always keeping the tax block of a bill that carries tax; a remaining NULL falls back to today's rule. P4-23 test 5, which asserted the old behaviour, now asserts the issued bill does not change. 7 tests; four deliberate breaks each caught. **Production order:** `p4c01-apply.mjs` (check, then `--apply`) **before** deploying the code — the new `finaliseBill` writes these columns — then deploy and restart, then `--apply` again (idempotent, fills bills finalised in between), then `p4c01-verify.mjs`.

  - **Review:** (2026-09-25) The migration was checked line by line and applied twice to the test database (verify ALL OK); the backfill's "settings as they stood at `finalised_at`" logic was exercised in isolated tables (before any audit row, a same-instant change, GST off with tax, a draft, a credit note, an already-set bill) — all correct. Two fixes. (1) `finaliseBill` recorded `issued_gst` from `gst_enabled` alone while the backfill uses `gst_enabled OR tax > 0`, and pricing and the snapshot read settings separately — a settings change between them could print tax with no GSTIN; it now uses the same rule. (2) **Deploying the code before the migration would have stopped all billing** — every finalise, bill PDF and credit note failed with "column issued_gst does not exist"; `finaliseBill`, `insertNote` and `billView` now check the columns first (new `issuedGst.js`, the same readiness pattern as `discountRules.js`), so **either order works**, and the second `--apply` fills anything finalised in between. A new test drops the columns inside a rolled-back transaction and finalises, prints and credits a bill; two deliberate breaks each failed it.

- [x] **P4C-03 · Realtime in a real browser** — `Done`
  - **Where:** `e2e/setup/localRealtime.mjs`, `e2e/billing/phase4c/P4C-03-realtime-in-a-real-browser.spec.js`, `src/queries/hooks/useGiniflowLive.js`, `src/queries/hooks/useDeskRequestsLive.js`, `src/lib/giniflowRealtime.js`.
  - **What:** prove a `billing_request` broadcast reaches a real browser and refreshes the screen, and that the 15 s poll still carries the change when realtime is off — without touching the production Supabase project, the only one there is.
  - **Result:** Done 2026-09-25 (built by a sub-agent). A local stack — the official Supabase Postgres 17.6.1.132 and Realtime v2.124.2 containers bound to 127.0.0.1, a random JWT secret, the real `2026-09-02_giniflow_realtime_rls.sql` applied unchanged, a localhost proxy doing Kong's routing, and a separate API (:3111) and Vite (:3110) on the test database with every production variable blanked — runs the real client path end to end. The RLS policy refuses both topics before the migration and admits a server-minted token to `giniflow:station:billing-requests` and `giniflow:day:<today>` after it: **no migration change needed**. In Chrome a request sent from the counter reached the open inbox in 115–287 ms, and an approval turned the counter's "My requests" to Approved in 376–556 ms, both shown on the page's own WebSocket frames to beat the poll; with realtime off the poll brought each change in about 15 s. **Bug fixed:** the Billing Counter called `useGiniflowLive()` with no date, so it joined no realtime channel at all; it now also joins the billing-requests station. 4 tests.
  - **Review:** (2026-09-25) The builder's claims held: the harness cannot reach production (shell `SUPABASE_*` values are overwritten, the API runs behind the network blocker), and dropping `billing_request` frames in the browser makes tests 1–2 fail, so they prove delivery by realtime rather than the poll. Four things were wrong and are fixed. **(1) Serious, older than this task: realtime went dead after every other token refresh, on every Gini Flow floor screen.** On refresh the code removed its channels and rebuilt them, but realtime-js hands back the _existing_ channel while it is still leaving and won't resubscribe it, so once it closed nothing rejoined until the next refresh — realtime was off from minute 50 to 100 of every hour-long token cycle, the screens silently falling back to their poll. A refresh now calls `client.realtime.setAuth(newToken)`, which pushes the new token into the joined channels and leaves them in place; a channel is rebuilt only if the server has already closed it. Proven against real 90 s and 30 s token expiry (a broadcast arrived 71 ms and 38 ms after the refresh). (2) Starting a second copy of the local stack deleted the first one's containers and crashed; it now claims its port first and only removes containers it created. (3) The spec never checked the realtime URL handed to the browser; it now refuses before any browser opens unless it is local (a fake remote URL was refused). (4) Tests 1–2 skipped silently when the stack was down; they now print a SKIPPED line, and `E2E_REQUIRE_REALTIME=1` makes the skip a failure. Measured cost of the counter fix: one extra channel per dated screen, one extra socket on screens without a date, duplicate frames merged into no extra refetch, no leaked channels after rapid navigation. With the stack up: 4/4 twice; without it: 2 passed, 2 skipped loudly. **Not proven:** production's own Supabase settings — see the supervised check in `52-BILLING-FLOOR-TRIAL.md`.

- [x] **P4C-04 · Unpriced test on a paid order** — `Done` (found by the P4-38 rehearsal)
  - **Where:** `server/services/billing/payments.js` (`settleOrder`, `releaseOrder`, `receptionState`); `e2e/billing/phase4/P4C-04-unpriced-test-on-a-paid-order.spec.js`.
  - **Why:** paying ₹250 for HbA1c on an order that also held an unpriced ₹300 test marked the whole ₹550 order paid and released both tests to the lab — the settle wrote `amount_paid = amount_total − claim`, as if the bill covered the whole order.
  - **Result:** Done 2026-09-25 (built by a sub-agent). Reproduced first on the test database. Three designs weighed: refusing to settle or finalise (rejected — it blocks a paying patient and can't tell an unpriced test from one the desk removed); per-test gating (rejected — the gate and the sample are per order, so it needs gate-code changes); **writing only what the bill covers** (chosen). The settle now writes as paid the order total less the price of each order test with no live line on this bill, less the claim, so an order with an unpriced test stays `part_paid` and the lab gate stays shut until reception collects the rest or the test is priced and billed. **The gate cannot release the priced test on its own** — it waits with the unpriced one. A fully priced order is unchanged. A release still restores exactly what this bill wrote; when reception has since added cash for the uncovered part, a cancel or credit takes away only the bill's part. P4-42's guard accepts that topped-up state only up to the price of the tests the bill still doesn't charge for. No schema, gate, `clearPayment`, `labPayment.js` or `visitLines.js` change; P4-18's pins hold. 6 tests; the regression set (P4-15/16/17/18/40/41/42, all P4B, P4C-04) passed 78/78 twice; five deliberate breaks each failed a test. Needs an API restart.
  - **Review:** (2026-09-25) The idea is right — a paid bill releases only what it bills, the gate waits for the unpriced test, a fully priced order is unchanged, and a regular-priced order above its category line still opens. Three things were wrong. **The bill's next settle wiped cash reception had taken for part of the unpriced test** — ₹250 on the bill plus ₹100 at the desk went back to ₹250 at finalise, so reception would collect ₹300 again: ₹650 for a ₹550 order. The settle now treats its own settle plus a reception top-up as its own, keeps that money up to the unpriced amount, and a cancel or credit gives back only the bill's part. **One bill line covered a test ordered twice** — the name check counted both HbA1c rows as billed, so a ₹250 bill wrote ₹500 paid and opened the gate; repeated tests are now matched one by one against the lines' quantity. **A rejected reception claim left a paid draft stuck for good** — after an insurer rejection and cash at the desk, finalise said "already paid ₹550 at reception" and the paid line couldn't be removed; claims are now compared by their money effect (a rejected claim counts as none). Checked and left: nothing renames a catalogue test; a catalogue test has at most one item; an inactive item still matches; a ₹0 unpriced test is released because the order says ₹0; money never exceeds the total; P4-41/P4-42 still refuse double collection; cancel and credit restore exactly. 12 tests (6 new); each fix undone once failed its test. 84/84 twice across P4-15/16/17/18/40/41/42, all P4B and P4C-04. Needs an API restart.
  - **Still open:** **removing a line from a paid draft leaves the gate open** until the next payment or finalise (the removed test can be drawn unpaid) — `removeLine` and `changeQuantity` in `bills.js` should call `settleTestOrders` after they reprice (P4C-05).
  - **Open:** with `SCRIBE_BILL_TAKES_TEST_PAYMENTS=1` reception cannot collect an unpriced part (`refuseOrderOnBill` refuses, and the bill has no line for it), so **Settings → Services → Not priced must be empty before the valve goes on** — or `refuseOrderOnBill` must learn about uncovered amounts. An order split across two bills is still treated per bill.

- [x] **P4C-05 · Reception money after a line change** — `Done` (found by the P4C-04 review)
  - **Where:** `bills.js` (`removeLine`, `changeQuantity`, new `resettleTestOrders`, `lastLineOfOrder`); `visitLines.js` (`refuseOrderOnBill`); `payments.js` (exports `UNCOVERED_SQL` only); `e2e/billing/phase4/P4C-05-reception-money-after-a-line-change.spec.js`; P4C-04 test 10; P4-18's pin.
  - **Why:** (1) removing a test line from a paid draft, or lowering its quantity, left the order paid and the sample released until the next payment or finalise — **a removed test could be drawn with nobody paying**. (2) With `SCRIBE_BILL_TAKES_TEST_PAYMENTS=1`, reception could never collect an unpriced test on an order that also had a billed one.
  - **Result:** Done 2026-09-25 (built by a sub-agent). Reproduced first: ₹650 stayed paid after ABI was removed, ₹400 stayed paid after an order's only line was removed, ₹500 stayed paid after quantity 2 went to 1, and the valve refused the unpriced ₹300. `removeLine` and `changeQuantity` now re-settle the visit's test orders in the same transaction, so the order at once shows what the bill still covers and the gate shuts; a removed last line on an order releases it first, giving back exactly this bill's part and keeping reception's. The re-settle is skipped when the bill holds no money (an unpaid draft, a ₹0 Pensioner draft) and when a live line carries reception money, so a line change never absorbs or wipes reception's cash. `refuseOrderOnBill` now lets reception collect **only the part the bill doesn't charge for, and only once the bill has settled what it does** ("Pay HbA1c on this visit's draft bill first; reception then collects ₹300.00 for the tests the bill doesn't charge for"); a fully priced order is still refused. `clearPayment`, `getPaymentQueue` and `labPayment.js` are byte-identical; **P4-18's `refuseOrderOnBill` pin was re-taken on purpose** (`c1ea3665…` → `a8c67fe7…`). 10 tests; the regression set (P4-10, P4-15/16/17/18, P4-40/41/42, P4C-04, P4C-05, all P4B) 101/101 twice; seven deliberate breaks each failed a test. One automatic restore after a break edited the wrong spot in `draftAtCheckIn`; it was repaired by hand, and the orchestrator confirmed the rehearsal's consultation guard there is intact (P4-38b + P4-07 18/18). Needs an API restart.
  - **Review:** (2026-09-25) The main change is right: removing a line, changing a quantity and removing an order's last line put the order where P4C-04's rules say on a paid, part-paid, unpaid, ₹0 Pensioner and CGHS-claim draft, and reception's money survived every case, including a reception clear landing mid-removal. The valve could not be made to collect money the bill charges for (a different quantity, a test ordered twice, a cancelled bill's line, a claim part, a re-added test). `clearPayment`, `getPaymentQueue` and `labPayment.js` are byte-identical to HEAD. **Fixed:** on an unfinalised ₹0 Pensioner draft the valve said "pay on the bill first"; it now says "Finalise this visit's draft bill for HbA1c first; reception then collects ₹300.00…" (new test 11); P4-18's `refuseOrderOnBill` pin re-taken again (`a5954f99…`). **Found, not fixed** (in `payments.js` / `testCancel.js`, not the reviewer's files) → **P4C-06**: (1) an insurance claim reception raises on the unpriced part — which the valve now allows — blocks the draft for good once approved (`order_claim`), and removing the priced line then leaves the bill's ₹250 on the order with the gate open; `toppedUp` should count a claim reception added after a claim-free settle as reception's money (reachable only with the valve on); (2) a desk line change (bill → order) **deadlocks** with a floor test cancellation (order → bill) — reproduced as `40P01`, a failed request rather than bad data, and the same risk exists for `takePayments`; `cancelTestIn` should lock the visit's draft bills before the order (reachable today); (3) raising an earlier line's quantity leaves a later order paid though the bill, filling lines in order, no longer covers it (minor); (4) a refusal after a re-add names the order's first line and whole amount. 120/120 twice across P4-10, 15–18, 40–42, P4C-04, P4C-05, all P4B, P4-38b and P4-07.
  - **Still open:** while a live line carries reception money, a line change doesn't re-settle the other orders until that line is removed; a floor test cancellation deletes draft lines without re-settling; on an unfinalised ₹0 Pensioner draft the valve's refusal says "pay on the bill first" where it should say finalise; an order split across two bills is settled per bill.

- [x] **P4C-06 · Lock order and reception claims** — `Done` (found by the P4C-05 review)
  - **Where:** `server/services/giniflow/testCancel.js` (`lockVisit`, new `lockBillsOf`); `server/services/billing/payments.js` (`receptionPart`, `withoutBill`, `settleOrder`, `releaseOrder`, `receptionState`, new export `orderShares`); `bills.js` (`resettleTestOrders`); `e2e/billing/phase4/P4C-06-lock-order-and-reception-claims.spec.js`; P4C-04 tests 5 and 11.
...
- [x] **P4B-01 · Decide how refunds work** — `Done`
  - **Result:** Decided 2026-09-25 (plan Q14). Written up in
    `docs/gini-flow/52-BILLING-REFUNDS-PLAN.md`: **R1** money goes back the way
    it was paid, and an admin may choose another mode with a reason; **R2** the
    desk asks and a reception_admin or admin approves before money leaves;
    **R3** a credit note — its own `CN` number, the original bill untouched and
    still final — covers the whole bill or chosen lines and quantities. Cancel
    stays for unpaid bills only.

- [x] **P4B-02 · Migration: refunds and credit notes** — `Done (test database; not yet on production)`
  - **Where:** `server/migrations/2026-10-22_billing_refunds.sql`.
  - **What:** `payments.direction` allows `out` (only on a credit note; cash out carries a shift); `bill_lines.credited_line_id` with the credited quantity capped at the original's; `billing_requests.kind` adds `refund` with its lines, requested and approved mode (`as_paid` / cash / card / UPI) and the credit note; one pending refund per bill.
  - **E2E test:** `e2e/billing/phase4b/P4B-02-migration-refunds.spec.js` (9 tests, including a two-connection race).
  - **Result:** Done 2026-09-25 (built by a sub-agent in an isolated worktree, merged and verified on the main checkout). Additive and safe to re-run: the only drops are constraints re-added in the same statement. Rules needing only their own row are named CHECKs; rules needing another row are `BEFORE` triggers raising `check_violation`: `bills_credit_note_guard` (a bill's type and original never change; a credit note credits only a final invoice, locked `FOR SHARE`), `payments_direction_guard` (`out` only on a credit note, `in` only on an invoice), `bill_lines_credit_guard` (a credit line credits a line of its own invoice with the same item, locks it `FOR NO KEY UPDATE`, and caps total credited quantity at the line's quantity). `payments.direction` allows `in`/`out` under the same check name; `payments_reference_check` is unchanged and already covers card/UPI `out` rows; cash out carries a shift. `bill_lines.credited_line_id` (never live, never itself). `billing_requests` adds kind `refund` with `refund_lines` (jsonpath-checked), `requested_mode`/`approved_mode` (`as_paid`/cash/card/UPI — `as_paid` means back the way it came, for split refunds), `mode_reason` (required when the modes differ) and `credit_note_id` (a credit note of that very bill); one pending refund per bill. The `CN` series needs no DDL; P4B-03 must add `CN` to `BILL_SERIES`. **First run:** it applied cleanly to the test database, and the spec passed 9/9 twice, including a real two-connection race (the second desk waits, then is refused once the line is used up). **Not yet on production:** `p4b02-apply.mjs` (check, then `--apply`) and the read-only `p4b02-verify.mjs` are in the scratchpad.
- [x] **P4B-03 · Credit note service** — `Done`
  - **Result:** Done 2026-09-25 (built by a sub-agent in an isolated worktree, merged and verified). `server/services/billing/creditNotes.js`: `creditNoteIn` runs inside the approving transaction, locks the invoice (a final invoice only — a draft, a cancelled bill and a credit note are each refused in words) and credits each line **in proportion**, cumulatively in paise with half-up rounding in BigInt, across the discounts, the tax (CGST = SGST), the patient part and the claim or adjustment; taxable and actual are derived so both line checks hold exactly, and the credit that finishes a line takes the exact remainder — a fuzz of 20,000 random lines credited in partial steps met every check and summed exactly. The note is `final` with a `CN` number (`CN` added to `BILL_SERIES` and `seriesFor`); its lines are not live and point at the credited line; the note that leaves nothing to credit carries the invoice's round-off, so all notes add up to the invoice's payable. A fully credited line stops counting for never-twice; crediting a whole pending claim sets `claim_status = 'none'`; a cleared claim is refused (`claim_cleared`). `cancelBill` now refuses a credit note and an invoice that has one. The fixture sweep deletes credit notes before their originals. 4 tests. **Verified on the main checkout:** the five Phase 4b specs plus P1-23, P4-05, P4-13…P4-18, P4-22 and P4-40…P4-42 — 123/123 on the first run, then 57/57; two deliberate breaks (a refund no longer capped at what was paid; the drawer no longer subtracting cash out) each failed a test and were restored by md5.
- [x] **P4B-04 · Refund requests: ask and approve** — `Done`
  - **Result:** Done 2026-09-25. `createRefundRequest` (a bill, its lines — a missing quantity means what is left — or `whole_bill`, a reason, `requested_mode` defaulting to `as_paid`) checks the lines against the credit rules before saving and refuses a line listed twice, a price field and an unknown mode; one pending refund per bill (the unique index's 23505 turned into a 409). `approveRequest` on a refund changes the mode only with `mode_reason`, creates the credit note in the same transaction and links it; a failed approval leaves the request pending and takes no `CN` number. Rejecting needs a note. Announced like the other kinds. 5 tests.
- [x] **P4B-05 · Pay out** — `Done`
  - **Result:** Done 2026-09-25. `payOut(creditNoteId, { version, payments })` pays back at most what is **due**: min(the note's payable less what has gone out on it, max(0, held − owed)), where held is money in less money paid out on all its notes and owed is the invoice's payable less all notes' payable — so on a pay-later bill a credit reduces the balance first, and only money actually paid comes back. `as_paid` splits by the original payments' modes, newest first, after earlier pay-outs; another approved mode accepts only that mode. Card/UPI need the reversal reference; cash needs the desk's open shift, locked, with enough in the drawer. Rows are `direction = 'out'`. Every `direction = 'in'` assumption in `payments.js` was updated (balance, `paid_amount`, `listPayments`, `listDues` net of credits and refunds). 4 tests.
- [x] **P4B-06 · Cash shift includes cash out** — `Done`
  - **Result:** Done 2026-09-25. Expected cash = opening + cash in − **cash out**; the shift view adds `refunded { cash, card, upi, total }`, `refund_count` and `credit_note_count`; a close that would record a negative drawer is refused in words. 2 tests.
- [x] **P4B-07 · Test orders on a credit note** — `Done`
  - **Result:** Done 2026-09-25. A fully credited test line whose sample is not yet taken puts its order's money back through `releaseTestOrders` (now able to take a list of orders, still reverting only while the order's money is exactly what this bill's settle left), once no live line still points at the order — so the gate closes. A test already done is credited only when the admin's approval carries a note (`test_done` otherwise), and its order is not reverted. 2 tests.
- [ ] **P4B-08 · Credit note PDF and refund receipt** — `Pending`
- [ ] **P4B-09 · Schemas and routes** — `Pending`
- [ ] **P4B-10 · Counter: Refund… and pay out** — `Pending`
- [ ] **P4B-11 · Desk requests inbox: refunds** — `Pending`
- [ ] **P4B-12 · Dues and bill views show credits and refunds** — `Pending`
- [ ] **P4B-13 · Smoke script, full suite, plan status** — `Pending`

## Each task's detail is in `52-BILLING-REFUNDS-PLAN.md` ("Tasks (Phase 4b)").
...
- [x] **P5-01 · Migration: claim settlements** — `Done`
  ...
  - **Result:** Done 2026-09-25 (built by a sub-agent). `2026-10-24_billing_claim_settlements.sql` creates `claim_settlements` (payer, date received, UTR reference, amount > 0, note, who cleared it, and void fields that are empty together or set with a reason) and `claim_settlement_bills` (one row per bill per payment, amount > 0, with a partial unique index so **a bill is in at most one live payment**), and adds `bills_claim_settlement_id_fkey` (ON DELETE RESTRICT) if missing. RLS on and forced, no anon/authenticated access. Existing bills pass because no bill had a settlement id (the apply script refuses if one does). P4-14 and P4B-03 now insert a real settlement instead of a random id, and the P4 fixture sweep removes settlements. 5 tests. **Applied to the test database only**; `p5-01-apply.mjs` / `p5-01-verify.mjs` are in the scratchpad for production.

- [x] **P5-02 · Pending register service** — `Done`
  ...
  - **Result:** Done 2026-09-25. `listPending` / `listCleared` in `cghsRegister.js` return every §8.3 column — bill, date, patient, UHID, sub-category, payer, doctor (from the line, else the appointment, else the visit), bill codes, the masked referral number, and the claim **net of final credit notes**, with days pending. Filters: date, sub-category, doctor, payer, and reference on the Cleared tab. Totals cover the whole filtered set. The pending total equals the stored claims. 5 tests.

- [x] **P5-03 · Clear one or many bills** — `Done`
  ...
  - **Result:** Done 2026-09-25. `clearBills` locks the chosen bills in id order (no deadlocks) and requires each to be final, pending and from one payer; the amount must equal the net claims exactly — **partial receipts are refused with the difference in words**, since CGHS pays in full (Q29/D14). A date received in the future or before a bill's date is refused. It writes one settlement and one link per bill, sets the bills cleared, re-runs the test-order settle (idempotent) and audits. `undoClear` — admin only, reason required — returns the bills to pending and voids the payment and its links; test orders are left alone because the claim is still owed. Two people clearing the same bill at once: one succeeds, the other is refused naming the reference. 10 tests.

- [x] **P5-04 · Register export** — `Done`
  ...
  - **Result:** Done 2026-09-25. Pending and Cleared `.xlsx` exports use the current filters, with a total row and a "By sub-category and doctor" sheet; the page also has a print view with the same subtotals. Export totals equal the screen totals. 4 tests.

- [x] **P5-05 · CGHS register routes and page** — `Done`
  ...
  - **Result:** Done 2026-09-25, apart from the counter badge. `server/routes/billingClaims.js` — seven `${BASE}` routes behind `BILLING_CLAIMS`, undo also needing `ADMIN` (P1-38 updated to match). The page `/billing/cghs-register` with a "CGHS register" nav entry for admin and reception_admin only: Pending and Cleared tabs, filters, totals, clear one or many with the live difference shown and Save held until it is zero, and admin undo with a reason. Bill reads now carry `claim_cleared_on`; the counter's "Cleared on <date>" badge is a two-line patch for the counter's owner, then an API restart. 6 tests, 5 passing until that lands.

  - **Review (P5-01…P5-05):** (2026-09-25), on the test database and over real HTTP. The server side held: the claim net of final credit notes is the one figure behind the list, totals, export, clear amount and print subtotals, in whole paise (three ₹333.33 claims plus a bill with two part-quantity credit notes agree to the paisa; the gross amount is refused with the difference); bills lock in id order and two desks clearing {A,B} and {C,B} at once never deadlocked in 6 rounds; cancelled, credited, ₹0-after-credit and mixed-payer selections are refused and a refusal writes nothing; undo is admin-only on server and screen, needs a reason, is refused twice, allows re-clearing; the migration runs twice, its apply script refuses when it should, verify ALL OK; reception is 403 on every route. Two things were wrong on the page. **The register's print CSS blanked every other print in the app** — `body * {visibility:hidden}` stays loaded after leaving the register, so the visit Rx, OPD report, dashboard and assess prints came out empty (40 visible lines, then 0); it is now scoped to `:where(body:has(.cghs-print)) *`. **"Select all filtered" over 500 bills let the desk fill the dialog and then be refused**, and printing a list cut off at 2,000 rows printed a Total the rows didn't add up to; the dialog now explains the 500-bill limit (shared `CLAIM_BILLS_AT_ONCE`) and holds Save, and Print is off while the list is cut off. 3 new tests; each fix undone once failed its test. **Product gap:** the exact-amount rule leaves no honest way to clear bills if CGHS pays short (TDS, disallowed items) — to be confirmed with the accounts team.

### 5B. Dashboards

- [x] **P5-06 · Reports service** — `Done`
  ...
  - **Result:** Done 2026-09-25 (built by a sub-agent). Every report runs through one `cleanFilters` (called once, in `runReport`, `server/services/billing/reportsFilters.js`) and `reportScope`: IST days, a 366-day cap (a blank `from` allowed only on dues and receivables), group/subgroup codes ignoring case, a category matching itself or its parent. A filter a report doesn't offer is refused in words, and a query with no column for a filter it offers throws — which caught one real mistake (receivables had offered group filters it couldn't apply). The spec checks every section of every report obeys the day range and every filter it offers.

- [x] **P5-07 · Revenue reports** — `Done`
  - **What:**
  ...
  - **Result:** Done 2026-09-25. Revenue by group › subgroup › item (whole range, or per day, week or month), by consultant (consultations / tests / other; the line's doctor, else the appointment's, else the visit's) and by category › sub-category (actual, discount, tax, round-off, patient share, collected, to be claimed, adjusted). **Only final bills count; a credit note counts on its own date with a minus sign**, so every figure is net. Each total equals the sum of its lines and every subtotal its children; totals are summed in SQL, never in JavaScript.

- [x] **P5-08 · Collection and dues reports** — `Done`
  - **What:**
  ...
  - **Result:** Done 2026-09-25. Collections by mode, user, shift and day show received, paid back and **net = in − out**, matching the payments exactly across the IST day boundary. Dues match `listDues` (the balance after credits and refunds) and say so when pay-later is off everywhere.

- [x] **P5-09 · Discount report** — `Done`
  ...
  - **Result:** Done 2026-09-25. Discounts by rule/code, method, category › sub-category, consultant and applying user; every total equals `bill_line_discounts` on final invoices; the per-group section shows discount as a % of actual, the discount credited back and the net %.

- [x] **P5-10 · Receivables, cancellations and requests reports** — `Done`
  - **What:**
  ...
  - **Result:** Done 2026-09-25. CGHS receivables (pending net of credit notes per line, by sub-category, consultant and ageing 0–30 / 31–60 / 61–90 / 90+; cleared per month from `claim_settlements`, skipping voided payments and links); coupon usage against its limits with an over-limit flag; cancellations with reasons; desk requests by user and kind.

- [x] **P5-11 · Excel export** — `Done`
  ...
  - **Result:** Done 2026-09-25. Every report downloads as `.xlsx` with the same filters — a Filters sheet, one sheet per section, and a total row equal to the API to the paisa (a break rounding to whole rupees was only caught once the seed had paise in it, so it now does). Tested for all 10 reports through the API and in the browser.

- [x] **P5-12 · Reports routes and page** — `Done`
  ...
  - **Result:** Done 2026-09-25. `server/routes/billingReports.js` (`${BASE}` on every route, `BILLING_REPORTS`): the catalog, one report, and its export. The page `/billing/reports` — a filter bar, a tab per report, Download Excel, a menu entry. Admin and reception_admin see it; reception, coordinator, lab, consultants and anonymous get 403 and no menu entry. A page shows at most 2,000 rows per section (totals always cover every row). **Performance** on a seeded year (≈110,000 bills, 330,000 lines, rolled back afterwards): revenue by service 8 s cold, 2.7–3.3 s warm; dues 4 s; the rest 0.3–1.6 s. 29 tests across P5-06…P5-12, twice; five deliberate breaks each caught.

  - **Review (P5-06…P5-12):** (2026-09-25, by a sub-agent) Every report was recomputed with **independent SQL** straight from bills, lines, payments, discounts and settlements, over a seeded period built from awkward cases — a partial credit note, a line credited twice on different days, a refund split part cash / part UPI, a partly credited CGHS claim later cleared, a voided settlement, a cancelled bill, a draft with a payment, pay-later part payments on later days, a ₹0.50 round-off, and payments at 23:59 and 00:01 IST. **Every report matched to the paisa**, with no double counting. Filter combinations, IST bounds, the 366-day cap, the row cap (with totals still over every row), `BILLING_REPORTS`-only access over real HTTP, and formula-safe exports (a patient named `=HYPERLINK(...)` exports as text) all hold. Two fixes: **the dues report hid real balances whenever pay-later was switched off** — it said no bill could be left with a balance, which is false for bills left unpaid while it was on; it now lists them ("Pay later is off now — these balances were left while it was on"); and the category report's "Collected" (money by the bill's date) differs from the Collections report's net (money by the day taken) for the same range — both are right, and the page now says so. P5-06…P5-12 29/29 twice after the fixes. **Open:** the coupon report may apply the per-doctor daily limit to "No consultant" rows, which enforcement skips (suspected, not proven); no index recommended yet — the evidence wasn't clean. The reviewer's performance probe held a long transaction on the shared test database and blocked other runs until it was ended; the probe specs were removed so the suite never runs them.

### 5C. Checks

- [x] **P5-13 · Smoke script: claims and reports** — `Done`
