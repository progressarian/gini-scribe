                          ...
                          {[...allMarkers.values()].map((mk,i) => {
                            ...
                            return (
                              <tr key={i} style={{ borderBottom:"1px solid #f1f5f9", background:i%2===0?"white":"#fafbfc" }}>
                                ...
                                <td style={{ padding:"8px", textAlign:"center" }}>
                                  <span style={{ fontSize:11, fontWeight:700, color:"white", background:st.bg, padding:"3px 10px", borderRadius:5, whiteSpace:"nowrap" }}>
                                    {st.icon} {st.label}
                                  </span>
                                </td>
                                <td style={{ padding:"8px" }}>
                                  <div style={{ display:"flex", flexWrap:"wrap", gap:2 }}>
                                    {mk.meds.length > 0 ? mk.meds.slice(0,3).map((m,mi) => (
                                      <span key={mi} style={{ fontSize:9, padding:"2px 6px", background:"#eff6ff", color:"#2563eb", borderRadius:3, fontWeight:600 }}>
                                        {m.name} {m.dose||""}
                                      </span>
                ...
                <div style={{ padding:"14px 16px", background:"white" }}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                    {[
                      ...
                      { key:"sideEffects", label:"⚠️ Side Effects", opts:["None","Mild","Significant","Needs change"], colors:["#059669","#d97706","#dc2626","#dc2626"] },
          ...
          {!dbPatientId ? (
            ...
            <div>
              ...
              {outcomesData && (() => {
                ...
                // Helper: find nearest visit date for context (labs may have dates that don't match any visit)
                ...
                const findNearest = (dateKey, dates) => {
                  ...
                  let best = dates[0], bestDiff = Math.abs(new Date(dateKey) - new Date(dates[0]));
                              ...
                              {isExpanded && (
                                <div style={{ borderTop:"1px solid #f1f5f9", padding:14, background:"#fafbfc" }}
                                  ...
                                  {c.data.slice().reverse().map((dp, di) => {
                                    ...
                                    const ctx = visitCtxByDate[dateKey] || visitCtxByDate[findNearest(dateKey, visitDates)] || {};
                                    const nearestMedDate = findNearest(dateKey, medDates) || dateKey;
                                    const allMeds = allMedsByDate[dateKey] || allMedsByDate[nearestMedDate] || [];
                ...
                return (
                  <>
                    {renderSection("Diabetes & Metabolic", "🩸", "#b91c1c", [
                      ...
                      { data:outcomesData.heart_rate, label:"Heart Rate", unit:" bpm", color:"#e11d48", valueKey:"pulse", biomarkerKey:"hr" },
                    ])}
                    ...
                    {renderSection("Body Composition", "🏋️", "#059669", [
                        { data:outcomesData.bmi, label:"BMI", unit:" kg/m²", color:"#0d9488", target:25, valueKey:"bmi", biomarkerKey:"bmi" },
