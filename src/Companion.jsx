...
export default function Companion() {
  ...
  const loadPatients = async () => {
    try {
      const r = await api.get("/api/patients?limit=50");
      setPatients(r.data);
    } catch (e) {
    ...
    >
      {[
        ...
        <button
          key={id}
          onClick={() => {
            if (id === "capture" && !selectedPatient) {
              alert("Select a patient first");
              return;
            }
            setScreen(id);
            if (id === "capture") setCaptureStep("camera");
          }}
          style={{
            flex: 1,
            padding: "8px 4px",
            border: "none",
            cursor: "pointer",
            background: screen === id ? "#eff6ff" : "white",
            display: "flex",
        ...
        >
          ...
          <span
            style={{ fontSize: 9, fontWeight: 700, color: screen === id ? "#2563eb" : "#94a3b8" }}
          >
            {label}
          </span>
        </button>
              ...
              {filtered.slice(0, 30).map((p) => (
                ...
                >
                  ...
                  <div style={{ textAlign: "right" }}>
                    ...
                    {p.last_visit && (
                      <div style={{ fontSize: 8, color: "#94a3b8" }}>{fDate(p.last_visit)}</div>
                    )}
                  </div>
                </div>
              ))}
          ...
          <div style={{ padding: 12 }}>
            ...
            {captureStep === "categorize" && currentCapture && (
              <div>
                ...
                <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 8, color: "#1e293b" }}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                  ...
                  <button
                    ...
                    style={{
                      flex: 2,
                      padding: "10px",
                      background: currentCategory ? "#2563eb" : "#cbd5e1",
                      color: "white",
                      border: "none",
                      borderRadius: 8,
                      fontSize: 12,
                      fontWeight: 800,
                      cursor: currentCategory ? "pointer" : "default",
                    }}
                  >
                    🧠 Extract with AI
                  </button>
            ...
            {captureStep === "extracting" && (
              <div style={{ textAlign: "center", padding: 40 }}>
                <div style={{ fontSize: 40, marginBottom: 12, animation: "pulse 1.5s infinite" }}>
                  🧠
                </div>
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 10, opacity: 0.7 }}>
                  {selectedPatient.age}Y/{selectedPatient.sex?.[0]} • {selectedPatient.file_no}
                </div>
              </div>
              <button
                onClick={() => {
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    ...
                    color: patientTab === id ? "#1e293b" : "rgba(255,255,255,.6)",
                  }}
                >
                  {label}
                </button>
                  ...
                  {(patientData.medications || []).filter((m) => m.is_active).length === 0 ? (
                    ...
                    >
                      {(patientData.medications || [])
                        ...
                        .map((m, i) => {
                          const con =
                            (patientData.consultations || []).find(
                              (c) => c.id === m.consultation_id,
                            ) || {};
                          return (
                            <div
                              key={i}
                    ...
                    (patientData.consultations || []).map((con, i) => (
                      <div
                        ...
                        style={{
                          ...
                          marginBottom: 6,
                          border: "1px solid #e2e8f0",
                          borderLeft: `3px solid ${con.status === "completed" ? "#059669" : con.status === "historical" ? "#7c3aed" : "#f59e0b"}`,
                        }}
                      >
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                          <div>
                            <div style={{ fontSize: 12, fontWeight: 700 }}>
                              {con.con_name || con.mo_name || "—"}
                            </div>
                            <div style={{ fontSize: 10, color: "#64748b" }}>
