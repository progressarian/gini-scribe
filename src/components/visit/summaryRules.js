import { getLabVal, getLabHist } from "./helpers";
...
// Case-insensitive partial match against med name
...
// Check diagnosis by id or label keyword
...
// ── Rule engine ───────────────────────────────────────────────────────────────
//
// Input:  { patient, diagnoses, activeMeds, stoppedMeds,
//           labResults, labHistory, vitals, documents }
// Output: { red: Alert[], amber: Alert[], green: Alert[] }
//
// Alert shape: { id, title, detail?, action? }
...
export function runSummaryRules({
  ...
  // R8: UACR worsening trajectory (3 consecutive, latest > 60)
  if (uacrH.length >= 3) {
    const [a, b, c] = uacrH.slice(-3).map((v) => parseFloat(v.result));
    if (c > b && b > a && c > 60 && !isNaN(a) && !isNaN(b) && !isNaN(c)) {
      // Only show if R1 didn't already flag this UACR (R1 is more actionable)
  ...
  // A1b: TSH overdue in hypothyroidism or on Levothyroxine
  ...
  // A: BP borderline elevated in hypertensive patient (not already red)
  if (
    isHTN &&
    latestV?.bp_sys != null &&
    latestV.bp_sys >= 130 &&
  ...
  // ── Prep-based rules (coordinator data) ──
  ...
  if (prep.medPct != null && prep.medPct < 50) {
    red.push({
      ...
      detail:
        "Patient is missing more than half their doses. Glycaemic and clinical cover is at risk.",
  ...
  // A6: Symptoms reported by coordinator since last visit
  ...
  // G4: Near remission (HbA1c < 6.5 for 2+ consecutive visits) — highest priority green
  let g4Fired = false;
  if (hba1cH.length >= 2) {
  ...
  // G1: Strong HbA1c improvement (> 1.5% drop since last visit)
  let g1Fired = false;
  if (hba1cH.length >= 2) {
    const [prev, curr] = hba1cH.slice(-2).map((v) => parseFloat(v.result));
  ...
  // G5: BP well controlled in a hypertensive patient
