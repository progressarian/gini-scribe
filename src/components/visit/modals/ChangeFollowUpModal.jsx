    ...
    <div className="mo open" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="mbox">
        ...
        <div className="mf">
          ...
          <textarea className="mta" style={{ minHeight: 55 }} placeholder="e.g. Repeat labs before visit..." value={notes} onChange={(e) => setNotes(e.target.value)} />
        </div>
        <div className="macts">
          <button className="btn" onClick={onClose}>Cancel</button>
          <button className="btn-p" disabled={!date} onClick={() => onSubmit({ date, notes })}>Save Date</button>
        </div>
      </div>
    </div>
  );
