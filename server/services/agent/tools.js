...
const UUID_RE_T = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
...
async function withSinceContext(pool, patientId, args) {
  if (args.since_last_visit && !UUID_RE_T.test(String(patientId ?? ""))) {
...
// ── create_health_log for UUID (app DB) patients ────────────────────────
// Column names differ from the hospital pool schema:
//   hospital: bp_systolic/bp_diastolic, weight_kg, patient_symptom_log + log_date
//   app DB:   systolic/diastolic,       weight,     symptom_logs        + logged_at
async function executeCreateHealthLogForAppDb(uuid, input) {
  const db = getGenieDb();
  if (!db) return { ok: false, error: "App DB not configured." };
  ...
  try {
    ...
    return { ok: false, error: e?.message || "Unexpected error saving to app DB." };
  }
}

// ── App DB (Supabase) queries for UUID patients ─────────────────────────

async function queryAppPatientData(uuid, scope, args = {}) {
  const db = getGenieDb();
  const noRecords = { records: [], total: 0, scope };
  if (!db) return noRecords;
  ...
  try {
    ...
    return { records: [], total: 0, scope, error: e?.message };
  }
  return noRecords;
}
...
async function getAppMedSchedule(uuid) {
  const db = getGenieDb();
...
export async function executeTool(name, input, ctx) {
  ...
  // App-only patients have a UUID — query the Supabase genie DB instead of
  // the hospital pool (which has an integer patient_id column).
  if (UUID_RE_T.test(String(scribePatientId ?? ""))) {
    return uuidPatientTool(name, input || {}, scribePatientId);
  }
