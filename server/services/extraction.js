...
- visit_date: ONLY extract if an explicit specific date is clearly written (e.g. 12/03/2024, 12-Mar-2024). Do NOT infer, assume, or use today's date. If no specific date is written or only vague terms like "today" are used, set visit_date to null AND return empty arrays for medications, diagnoses, stopped_medications, advice and null for vitals.bp_sys / vitals.bp_dia / vitals.pulse, follow_up and follow_up_with — do not extract any data without a confirmed written date. EXCEPTION: vitals.height and vitals.weight may STILL be extracted when no date is written, IF they appear in a clearly labelled vital-signs header/footer block at the top or bottom of the prescription (e.g. "VITAL SIGNS: H:- 161 CM, W:- 87.15 KG", "Ht: 161cm  Wt: 87.15kg"). These standing measurements are treated as current and will be stamped with today's date downstream. Do NOT apply this exception to height/weight values that are only mentioned inline in clinical notes or follow-up sections.
- Name must be in English/Roman script
- follow_up_with: Capture the free-text block that appears under a heading like "FOLLOW UP WITH", "Follow up with", or "Next visit instructions" — typically lists tests/preparations the patient must bring or do for the next visit (e.g. "FASTING SAMPLE AT 8:30AM AFTER OMISSION OF ANTIDIABETIC MEDICATION FOR 24 HRS — FBG, HBA1C, LIPIDS"). Preserve original line breaks using \\n. Do NOT include the next-visit date itself (that maps to follow_up). Set to null if no such block is present.
