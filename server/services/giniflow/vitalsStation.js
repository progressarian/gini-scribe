...
//
// Queue order follows the prototype: the patient at the station is "Now", the
// one after is "Next", the rest show their appointment time.
...
export async function getVitalsQueue(visitDate, db = pool) {
  const { rows } = await db.query(
    `SELECT v.id, v.current_status, v.category, v.appointment_time::text AS appointment_time,
       ...
       LEFT JOIN LATERAL (
         SELECT a.biomarkers FROM appointments a
  ...
  const ordered = [...rows].sort((a, b) => {
    const atStation = (r) => (r.current_status === "with_vitals" ? 0 : 1);
    return atStation(a) - atStation(b);
  });
  ...
  return {
    ...
    queue: ordered.map((r, i) => ({
      ...
      slot: i === 0 ? "Now" : i === 1 ? "Next" : (r.appointment_time || "").slice(0, 5) || "—",
