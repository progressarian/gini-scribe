...
- For labs: extract ALL lab values with test name, numeric value, unit. Include HbA1c, FBG, PPBG, LDL, TG, HDL, Non-HDL, Cholesterol (Total), TSH, T3, T4, Creatinine, eGFR, UACR, Hb, Iron, Ferritin, OT/SGOT, PT/SGPT, ALP, Calcium, Albumin, GTT, Insulin, C-Peptide, HOMA-IR, HOMA-Beta, Uric Acid, FIB4, Vitamin D, Vitamin B12, AMH, Testosterone, DHEAS, Prolactin, LH, FSH, Estradiol, Progesterone, FPI, Amylase, Lipase, Fecal Elastase (FE), VPT (Vibration Perception Threshold — extract R and L values separately as "VPT Right" / "VPT Left"), ABI (Ankle-Brachial Index — extract as "ABI Right" / "ABI Left"), Hirsutism Score/FGS/H.Score, Potassium, Sodium, Anti-TPO, Anti-Tg, Anti-tTG (tTG), etc.
  ...
  EXCEPTION — OBSERVATIONS SKIPPED WHEN MATCHING FOLLOW UP EXISTS: If the user message begins with "Visit date: YYYY-MM-DD" AND the note contains a "FOLLOW UP ON <date>" section where <date> matches that visit date → do NOT assign date "today" to OBSERVATIONS labs. Skip OBSERVATIONS labs entirely — the FOLLOW UP section for the visit date is the authoritative source and OBSERVATIONS is the historical baseline.
  CRITICAL — USE INTELLIGENCE TO DETECT CARRIED-FORWARD HISTORICAL TEXT. HealthRay clinical notes frequently copy prior visits' notes verbatim into a new note. The word "TODAY" inside a sub-block does NOT always mean the current visit — it can mean "today" as written when that block was originally authored. Apply this judgment:
    ...
    • If "TODAY" / "OBSERVATIONS" is the FIRST or PRIMARY block in the note and there is no older dated header above it, treat it as the current visit → date: "today". EXCEPTION: If the user message starts with "Visit date:" and the note has a matching "FOLLOW UP ON <visit-date>" section, this bullet does NOT apply — skip OBSERVATIONS labs entirely.
...
- OBSERVATIONS BLOCK vs SAME-DATE FOLLOW UP — VISIT DATE ANCHOR: The user message may begin with a line "Visit date: YYYY-MM-DD". When present, use it to resolve whether an undated OBSERVATIONS block belongs to the current visit or is a historical baseline:
  • ABSOLUTE RULE — If a "FOLLOW UP ON <date>" block exists where <date> equals the visit date: use ONLY the data (labs, vitals, medications) present in that FOLLOW UP section. The OBSERVATIONS block is the enrollment-time baseline and MUST BE COMPLETELY IGNORED. Do NOT extract any field from OBSERVATIONS — not UACR, not Creatinine, not HbA1c, not any test — even if that test is absent from the FOLLOW UP section. A test missing from the FOLLOW UP section means it was NOT measured on this visit. Do NOT backfill from OBSERVATIONS. Do NOT attribute OBSERVATIONS data to "today". The OBSERVATIONS block does not exist for the purpose of this extraction.
  • If the note contains NO "FOLLOW UP ON <date>" sections at all (first/fresh visit with no prior visit history in this note), treat the undated OBSERVATIONS block as the current visit prescription source — extract labs, vitals, and medications from it with date: "today".
  • If the note DOES contain "FOLLOW UP ON <date>" sections but NONE of them match the visit date, do NOT use OBSERVATIONS data — skip it entirely and leave those fields empty. The patient likely did not visit on this appointment date.
  • Apply this rule ONLY when the user message starts with "Visit date:". Without it, fall back to the existing date-attribution rules.
  ...
  EXAMPLE (no match — skip): Visit date: 2026-05-18. Note has "FOLLOW UP ON 20/3/26" and "FOLLOW UP ON 15/4/26" but NO "FOLLOW UP ON 18/5/26". OBSERVATIONS block has FBG-131, HBA1C-7.2, TREATMENT: TAB MED 500MG OD. Since the note has follow-up history but none matches 2026-05-18: OMIT OBSERVATIONS entirely — return empty arrays for labs, medications, vitals.
  EXAMPLE (first visit — use OBSERVATIONS): Visit date: 2026-05-18. Note has NO "FOLLOW UP ON <date>" sections at all — only an OBSERVATIONS block (FBG-200, HBA1C-9, TREATMENT: TAB METFORMIN 500MG OD). Since there are no follow-up sections: OBSERVATIONS IS the current visit — extract FBG-200 and HBA1C-9 as labs with date "today", and TAB METFORMIN as a current medication.
- CRITICAL — all dates in these notes are in DD/MM/YYYY format (Indian standard). "06/04/2026" means April 6 2026 → output as 2026-04-06. NEVER interpret as MM/DD/YYYY.`;
