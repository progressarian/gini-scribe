...
- For labs: extract ALL lab values with test name, numeric value, unit. Include HbA1c, FBG, PPBG, LDL, TG, HDL, Non-HDL, Cholesterol (Total), TSH, T3, T4, Creatinine, eGFR, UACR, Hb, Iron, Ferritin, OT/SGOT, PT/SGPT, ALP, Calcium, Albumin, GTT, Insulin, C-Peptide, HOMA-IR, HOMA-Beta, Uric Acid, FIB4, Vitamin D, Vitamin B12, AMH, Testosterone, DHEAS, Prolactin, LH, FSH, Estradiol, Progesterone, FPI, Amylase, Lipase, Fecal Elastase (FE), VPT (Vibration Perception Threshold — extract R and L values separately as "VPT Right" / "VPT Left"), ABI (Ankle-Brachial Index — extract as "ABI Right" / "ABI Left"), Hirsutism Score/FGS/H.Score, Potassium, Sodium, etc.
  ...
  BRIEF HISTORY section may contain historical HbA1c or glucose readings — extract these as real lab results with whatever date context is available (e.g. "HBA1C-8 IN SEPT,25" → test: "HbA1c", value: "8", date: "2025-09-01"). If no date given, use date: null. IMPORTANT: If the same test appears multiple times (e.g. HbA1c in OBSERVATIONS and in a follow-up section), extract EACH occurrence separately with its own date. The OBSERVATIONS section (no date header) should have date: null — do NOT skip it because a later follow-up has the same test. Extract every numeric lab value you see.
...
- For diagnoses: extract ALL conditions from the DIAGNOSIS section, both present and absent. Rules:
  ...
  • A "-" suffix directly after the name (before any space or parenthesis) means ABSENT → status: "Absent", strip "-" from name (e.g. "CAD-" → name: "CAD", status: "Absent"; "NEPHROPATHY-(G1A1)" → name: "NEPHROPATHY", details: "G1A1", status: "Absent"). Also mark Absent if details say "absent", "negative", "no history of", "ruled out", "(-)", "not present".
  ...
  • Conditions with NO sign listed INLINE on the SAME COMMA-SEPARATED LINE as absent "-" conditions (e.g. "CAD-, CVA-, PVD" — PVD has no sign but shares the absent line) → status: "Absent".
  • EXCEPTION: A condition with no sign on a mixed line that is a well-known diabetic complication (NEUROPATHY, NEPHROPATHY, RETINOPATHY) should be treated as "Present" ONLY if it appears on its own line or with "+" conditions. If it appears on a line with "-" conditions without its own sign, mark "Absent".
