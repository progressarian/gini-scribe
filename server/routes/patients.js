  ...
  try {
    ...
    if (q) {
      ...
      const exactHit = await pool.query(
        `SELECT 1 FROM patients WHERE file_no = $1 OR phone = $1 OR $1 = ANY(alt_phone) OR abha_id = $1 LIMIT 1`,
        [trimmed],
      );
      if (exactHit.rowCount > 0) {
        conditions.push(
          `(p.file_no = $${idx} OR p.phone = $${idx} OR $${idx} = ANY(p.alt_phone) OR p.abha_id = $${idx})`,
        );
        params.push(trimmed);
        idx++;
      } else {
        conditions.push(
          `(p.name ILIKE $${idx} OR p.phone LIKE $${idx} OR p.file_no ILIKE $${idx} OR p.abha_id ILIKE $${idx}
            OR EXISTS (SELECT 1 FROM unnest(COALESCE(p.alt_phone, '{}')) alt WHERE alt LIKE $${idx})
            OR EXISTS (
